...
def get_fare(intital_stop, destination_stop):
   BusStops = ['TH', 'GA', 'IC', 'HA', 'TE', 'LU', 'NI','CA']
   Path = [800, 600, 750, 900, 1400, 1200, 1100, 1500]
   x = -1
   y = -1
   for i in range(len(BusStops)):
      if intital_stop == BusStops[i]:
         x = i
      if destination_stop == BusStops[i]:
         y = i
   if x == -1 or y == -1 or x == y:
      print("INVALID INPUT")
      exit()
   if x < y:
      result = sum(Path[x + 1:y + 1]) * 0.005
   else:
      result = (sum(Path[x + 1:]) + sum(Path[:y + 1])) * 0.005 

   return float(math.ceil(result))

intital_stop = input()
destination_stop = input()
print(get_fare(intital_stop, destination_stop), "INR")